...
window.SDK_INITIALIZED.then(() => {
  wmeSDK = getWmeSdk({ scriptId: "wme-image-overlays", scriptName: "WME Image Overlays"});
...
async function onWmeReady() {
  ...
  // Show data source warning for lower-levelled editors
  if (localStorage.ImageOverlays_warning == undefined && wmeSDK.State.getUserInfo().rank < 3) {
    ...
    warningMessage.innerHTML = `<i class="fa fa-fw fa-3x fa-exclamation-triangle pull-left"></i>
    <h3>${I18n.t('image_overlays.warning_title')}</h3>
    <p>${I18n.t('image_overlays.warning_explanation1')}</p>
    <p>${I18n.t('image_overlays.warning_explanation2')}</p>
    <p style="margin-bottom:0">${I18n.t('image_overlays.safe_sources_header')}</p>
    <ul>${I18n.t('image_overlays.safe_sources_list')}</ul>
    <p>${I18n.t('image_overlays.community_advice')}</p>
    <button class="btn btn-success" disabled="disabled"><i class="fa fa-fw fa-check"></i> ${I18n.t('image_overlays.warning_button')} - <span>20</span></button>
    <footer>${I18n.t('image_overlays.warning_info')}</footer>`;
    const warningButton = warningMessage.querySelector('button');
    ...
    const warningCountdown = warningButton.querySelector('span');
    tab.appendChild(warningMessage);
    const tabActivityListener = new MutationObserver(() => {
      if (tab.parentNode.classList.contains('active')) {
        let intervalId = setInterval(() => {
          if (warningCountdown.textContent > 1) {
            ...
            warningButton.innerHTML = `<i class="fa fa-fw fa-check"></i> ${I18n.t('image_overlays.warning_button')}`;
          }
        }, 1000);
  ...
  const persistentStorageWarning = document.createElement('div');
  ...
  parentLayer.required = true;
  parentLayer.add(new Option("Satellite Imagery", "satellite_imagery"));
  parentLayer.add(new Option("Roads", "roads", true));
  parentLayer.add(new Option("Drives", "drives"));
  parentLayer.add(new Option("Area Managers", "area_managers"));
  parentLayer.add(new Option("Map Comments", "mapComments"));
  parentLayer.add(new Option("Venues", "venues"));
  parentLayer.add(new Option("Segments", "segments"));
  parentLayer.add(new Option("Editable Areas", "editable_areas"));
  parentLayer.add(new Option("Issues", "mapProblems"));
  parentLayer.add(new Option("House Numbers", "house_numbers"));
  ...
  versionInfo.href = 'https://www.waze.com/discuss/t/script-wme-image-overlays-1-5-4/129015';
  ...
  function displayAlignPage(overlay) {
    ...
    imageNameInput.value = overlay.name || overlay.blob.name;
  ...
  function pinToMap() {
    ...
    storeActiveOverlay((e) => {
      ...
      for (var i = imagesList.childNodes.length - 1; i >= 0; i--) {
        if (imagesList.childNodes[i].dataset.key == e.target.result) {
  ...
  function displayImageOverlay(overlay) {
    ...
    img.addEventListener('load', () => {
      ...
      parentLayer.value = overlay.layerTarget;
      // Old entries might have a layer set as target that wasn't hard-coded. Fall back to the most practical location for images
    ...
    img.src = window.URL.createObjectURL(overlay.blob);
  }
...
function setTranslations() {
  let translations = {
    en: {
      ...
      community_advice: 'When in doubt about a resource, contact <a href="https://www.waze.com/discuss/c/wazeopedia/5162">your local community</a> and ask for their advice.',
...
.image-overlays-source-warning button {
  display: block;
  margin: 0 auto;
}
